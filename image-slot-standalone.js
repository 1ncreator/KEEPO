...
(() => {
  ...
  function load() {
    ...
    // Standalone/offline export: an embedded JSON script tag (written by the
    // "Save as standalone HTML" flow) takes priority over the network fetch,
    // which won't resolve from a file:// or blob: document.
    let embedded = null;
    try {
      const tag = document.getElementById('__image_slots_embedded');
      if (tag) embedded = JSON.parse(tag.textContent);
    } catch (e) {}
    loadP = (embedded ? Promise.resolve(embedded) : fetch(STATE_FILE).then((r) => (r.ok ? r.json() : null)))
      .then((j) => {
